...
### 8.7: (Optional) Enable CORS for Frontend Apps

If your client app is hosted on a different domain, enable CORS:

```bash
# Enable CORS for Textract endpoint
...
# Enable CORS for Selfie endpoint
aws apigateway put-method-response \
  --rest-api-id $API_ID \
  --resource-id $SELFIE_RESOURCE \
  --http-method POST \
...
// Call Textract Lambda
async function verifyDocument(userId, frontPath, backPath) {
  try {
    const frontImage = imageToBase64(frontPath);
    const backImage = imageToBase64(backPath);

    const response = await axios.post(`${API_ENDPOINT}/textract`, {
      frontImage,
      backImage,
      userId
    });
...
// Call Selfie Lambda
async function verifySelfie(userId, selfiePath) {
  try {
    const image = imageToBase64(selfiePath);

    const response = await axios.post(`${API_ENDPOINT}/selfie`, {
      image,
      userId
    });

    console.log('Selfie Result:', response.data);
    return response.data;
  } catch (error) {
    console.error('Error:', error.response?.data || error.message);
  }
...
// Usage
verifyDocument('user123', './front.jpg', './back.jpg');
verifySelfie('user123', './selfie.jpg');
```
...
export function VerificationForm() {
  ...
  const handleDocumentUpload = async (event) => {
    const frontFile = event.target.files[0];
    const backFile = event.target.files[1];

    if (!frontFile || !backFile) return;

    setLoading(true);
    try {
      const frontImage = await fileToBase64(frontFile);
      const backImage = await fileToBase64(backFile);
  ...
  const handleSelfieUpload = async (event) => {
    ...
    setLoading(true);
    try {
      const image = await fileToBase64(selfieFile);
...
def verify_document(user_id, front_path, back_path):
    front_image = image_to_base64(front_path)
    back_image = image_to_base64(back_path)
    ...
    response = requests.post(f'{API_ENDPOINT}/textract', json=payload)
    return response.json()

def verify_selfie(user_id, selfie_path):
    image = image_to_base64(selfie_path)
    ...
    response = requests.post(f'{API_ENDPOINT}/selfie', json=payload)
    return response.json()

# Usage
if __name__ == "__main__":
    result = verify_document('user123', './front.jpg', './back.jpg')
    print(json.dumps(result, indent=2))

    result = verify_selfie('user123', './selfie.jpg')
    print(json.dumps(result, indent=2))
```
...
```bash
# Textract logs
aws logs tail /aws/lambda/textract-id-analyzer --follow --region us-east-1

# Selfie logs
aws logs tail /aws/lambda/selfie-verification --follow --region us-east-1
```
